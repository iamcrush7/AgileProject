    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-gray-400 text-sm mt-0.5">{users.length} total users on the platform</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm outline-none">
                    {["ALL", "USER", "PROVIDER", "ADMIN"].map((r) => <option key={r} value={r} className="bg-gray-900">{r === "ALL" ? "All Roles" : r}</option>)}
                </select>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="rounded-3xl border border-white/10 bg-white/5 overflow-hidden">
                    <div className="hidden md:grid grid-cols-12 gap-4 px-6 py-3 border-b border-white/5 text-xs font-bold text-gray-500 uppercase tracking-wider">
                    ...
                    {filtered.map((u, i) => (
                        <motion.div key={u.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                            className="grid grid-cols-1 md:grid-cols-12 gap-4 px-6 py-4 border-b border-white/5 last:border-0 hover:bg-white/5 items-center">
                            ...
                            <div className="md:col-span-3"><p className="text-gray-400 text-sm truncate">{u.email}</p></div>
                            ...
                            <div className="md:col-span-2"><p className="text-gray-500 text-xs">{new Date(u.createdAt).toLocaleDateString("en-IN")}</p></div>
                            <div className="md:col-span-1">
                                <button onClick={() => blockUser(u.id)} className="p-2 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors" title="Block user">
                                    <UserX size={16} />
                                </button>
                            </div>
                        </motion.div>
