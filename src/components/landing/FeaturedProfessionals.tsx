...
const Provider3DCard = ({ pro, idx, yOffset }: { pro: Pro, idx: number, yOffset: MotionValue<number> | number }) => {
            ...
            try {
                ...
                if (data.success && data.data && data.data.length > 0) {
                    const topPros = data.data
                        .sort((a: { rating?: string }, b: { rating?: string }) => parseFloat(b.rating || "0") - parseFloat(a.rating || "0"))
                        .slice(0, 4)
                        .map((p: { id: string, name?: string, businessName?: string, services?: { category: string }[], rating: number | string, reviewsCount: number, experience: number, city?: string, state?: string, image?: string }, idx: number) => ({
                            id: p.id,
