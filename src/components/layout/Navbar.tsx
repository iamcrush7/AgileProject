...
import { Menu, X, LayoutDashboard, CalendarRange, Settings, LogOut, Briefcase, Shield, ChevronDown, LucideIcon } from "lucide-react"
...
const roleMenus: Record<string, { label: string; href: string; icon: LucideIcon }[]> = {
    USER: [
...
function UserMenu() {
    ...
    return (
        <div ref={ref} className="relative">
            ...
            >
                <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary text-xs font-medium text-background overflow-hidden border border-border relative">
                    {session.user.image ? (
                        <Image src={session.user.image} alt={name} className="h-full w-full object-cover" fill />
                    ) : (
                        initial
                    )}
                </div>
