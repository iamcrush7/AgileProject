...
import { Mail, ArrowRight, ArrowLeft, CheckCircle, Loader2, Copy } from "lucide-react"
                        ...
                        <AnimatePresence mode="wait">
                            {status === "sent" ? (
                                ...
                                >
                                    ...
                                    <h2 className="text-xl font-extrabold text-gray-900 dark:text-white mb-2">Reset link ready</h2>
                                    <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed mb-4">
                                        If <span className="text-gray-900 dark:text-white font-semibold">{email}</span> is registered, the reset link is shown below.
                                    </p>
                                    {resetUrl ? (
                                        <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 text-left overflow-hidden">
                                            <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Open this link to reset your password:</p>
                                            <div className="flex flex-col gap-3">
                                                <div className="w-full overflow-hidden break-all whitespace-normal">
                                                    <a href={resetUrl} className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline break-words max-w-full">
                                                        {resetUrl}
                                                    </a>
                                                </div>
                                                <button
                                                    ...
                                                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 text-white px-3 py-2 text-sm font-semibold hover:bg-indigo-500 transition"
                                                >
                                                    <Copy size={16} />
                                        ...
                                        <p className="text-xs text-gray-400 dark:text-gray-600">If the email is registered, a reset link has been generated.</p>
                                    )}
                                </motion.div>
                            ) : (
                                <motion.form
