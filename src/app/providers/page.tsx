...
import { Search, MapPin, Star, Filter, ShieldCheck, ChevronDown, ArrowUpDown } from "lucide-react"
    ...
    const fetchProviders = useCallback(async () => {
        ...
        try {
            ...
            if (json.success) {
                ...
                if (verifiedOnly) {
                    filtered = filtered.filter((p: ProviderType) => p.verified)
                }

                if (minRating) {
                    filtered = filtered.filter((p: ProviderType) => {
                        const r = p.rating === "New" ? 0 : parseFloat(p.rating as string)
                        return r >= minRating
                    })
                }
                ...
                if (sortBy === "Highest Rated") {
                    filtered.sort((a: ProviderType, b: ProviderType) => {
                        const rA = a.rating === "New" ? 0 : parseFloat(a.rating as string)
                        const rB = b.rating === "New" ? 0 : parseFloat(b.rating as string)
                        return rB - rA
                    })
                } else if (sortBy === "Most Experienced") {
                    filtered.sort((a: ProviderType, b: ProviderType) => b.experience - a.experience)
                } else if (sortBy === "Lowest Price") {
                    filtered.sort((a: ProviderType, b: ProviderType) => a.startingPrice - b.startingPrice)
                }
                                    ...
                                    {providers.map((provider) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-wrap gap-2 mb-6">
                                                {provider.services?.slice(0, 2).map((s: ServiceType) => (
