...
import { MapPin, Star, ShieldCheck, ArrowLeft, ArrowRight, User, Filter, Clock, Briefcase, CheckCircle2 } from "lucide-react"
...
export default function ServiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [serviceInfo, setServiceInfo] = useState<ServiceInfoType | null>(null)
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const info = serviceData.services?.find((s: ServiceInfoType) => s.name === serviceName)
            setServiceInfo(info)
                        ...
                        {isLoading ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                {providers.map((p) => {
                                    ...
                                    const matchingService = p.services.find((s: ProviderServiceType) => s.name === serviceName)
                                            ...
                                            <div className="p-8">
                                                <div className="flex items-start gap-5 mb-8">
                                                    <div className="relative flex-shrink-0">
                                                        {p.image ? (
                                                            <Image src={p.image} alt={p.name} width={80} height={80} className="w-20 h-20 rounded-2xl object-cover ring-4 ring-surface" />
                                                        ) : (
