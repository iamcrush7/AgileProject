...
function BookingContent() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (pData.success) {
                ...
                const s = pData.data.services.find((s: ServiceType) => s.id === serviceId)
                setService(s)
            }
            // we don't use availability state anymore as per lint
        } catch (err) {
            console.error(err)
    ...
    }, [status, providerId, serviceId, serviceName, router, fetchData])
                    ...
                    <div className="bg-surface border border-border rounded-3xl p-8 sticky top-32 space-y-8">
                        <div>
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                {provider?.image ? (
                                    <Image src={provider.image} alt={provider.name} width={48} height={48} className="w-12 h-12 rounded-xl object-cover" />
                                ) : (
